...
test("architecture stays small, pure, direct, and dependency-free", async () => {
	...
	assert.ok(sourceLines <= 2902, `src has ${sourceLines} lines; audit against first principles`);
	...
	assert.match(
		main,
		/satisfies\s+Record<\s*"init" \| "workspace" \| "spawn" \| "continue" \| "steer" \| "stop" \| "wait" \| "jobs" \| "prune" \| "watch" \| "unwatch" \| "open" \| "close" \| "sweep" \| "linear"\s*,?\s*Command\s*>/,
	);
});
