...
test("architecture stays small, pure, direct, and dependency-free", async () => {
	...
	assert.ok(sourceLines <= 1450, `src has ${sourceLines} lines; audit against first principles`);
	assert.doesNotMatch(await readFile(join(ROOT, "src/job.ts"), "utf8"), /from ["']node:/);
	assert.deepEqual((await readdir(join(ROOT, "src/commands"))).sort(), ["init.ts", "jobs.ts", "migrate.ts", "spawn.ts", "stop.ts", "wait.ts", "watch.ts"]);
	...
	assert.match(main, /satisfies Record<"init" \| "workspace" \| "migrate" \| "spawn" \| "stop" \| "wait" \| "jobs" \| "watch" \| "unwatch"/);
});
