...
test("architecture stays small, pure, direct, and dependency-free", async () => {
	...
	assert.ok(
		sourceLines <= 4053,
		`src has ${sourceLines} lines; includes bounded finish-turn inspection, pruned-checkout continuation, empty-result finish suppression, same-tip finish quieting, spawn fail-closed on a ticket missing from the base commit, and hosted PATH plus HERDR_ENV`,
	);
