...
const DEFAULT_RERING_MS = 15 * 60_000;
...
async function sweepProject(root: string): Promise<void> {
	const jobs = join(root, ".limen", "jobs"),
		threshold = positive("LIMEN_SEAT_RING_MS", DEFAULT_RING_MS);
	...
	for (const entry of fs.existsSync(jobs) ? fs.readdirSync(jobs, { withFileTypes: true }) : []) {
		...
		if (Date.now() - Math.max(0, ...markers.map((name) => modified(join(seat, name)))) < positive("LIMEN_SEAT_RERING_MS", DEFAULT_RERING_MS)) continue;
