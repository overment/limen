...
The checked-in [root setup](seat/github-setup.sh) is the repeatable alternative to manually transcribing steps 2–6 below. On Alice, stop the existing timer before repairing any worker-owned binary; mode 0755 does not make worker-owned interpreter or Herdr code safe. `/usr/local/bin` and `/usr/local/bin/node` have been corrected to root ownership, but Herdr must be checked and repaired too. Stage trusted Node 24 and Herdr release binaries under root-owned paths first. **Never** link the poller interpreter to `~overment/.nvm`, run `limen github poll` as a proof, or enable the timer until both binaries and their parents are root-owned and not group/other writable.

For Alice x86_64, stage the sources as root from official releases before setup (versions verified 2026-09-25). Compare the Node archive checksum with [Node's published list](https://nodejs.org/dist/v24.19.0/SHASUMS256.txt) and the Herdr digest with [its release asset](https://github.com/herdrdev/herdr/releases/tag/v0.9.1); do not promote the worker-owned binaries already in `/usr/local/bin` into trusted sources:
