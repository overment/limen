...
Here **Alice** means the project checkout on its owning VPS, not the Limen package checkout at `/opt/limen` and not a Mac clone. Its Herdr coordinator is Tony's persistent shepherding lane. Do not edit `alice/` to provision the doorbell. The root-owned `/opt/limen` poller and the coordinator's `limen` CLI must run the **same landed version**: deliberately update both when deploying or upgrading; mismatched claim and command protocols fail closed rather than being treated as a successful handoff.
