...
| 4 | **F048** hosted runtime starts its own agent | ACTIVE | `spawn --tab` returns ID in seconds; detached supervisor starts pi + owns lifecycle (closes 2026-08-27 orphan) | `[~]` finish/proof starts from the existing supervisor-first implementation, not a rewrite |
...
- [ ] F091: observed completed-turn inspection path requires receiver-owned evidence source/contract; correlated Alice Mac proof is missing, and the landed transport portion is not feature completion
