...
|---|---|---|---|---|
| **Default (detached)** | `limen spawn …` | Background `pi` (JSON stream, timeout, tool-call cap, process containment) | Live **log tail** (`tail -f`) | Ordinary slices; leave it alone |
| **Hosted (interactive)** | `limen spawn --tab …` | Interactive `pi` **inside** the job tab | The worker itself — you can type | Supervision, takeovers, work that may go sideways |

Default stays detached. `--tab` is opt-in and requires Herdr (`HERDR_ENV=1`). Without Herdr, `--tab` refuses cleanly — use an ordinary job.
...
2. Start implementation with a short coordinator instruction, then a pointer to the ticket — do not dump the ticket as the whole prompt. Example: `limen spawn --label "FNNN short name" "Implement FNNN: <one-sentence outcome>. Start by writing <first file or slice>. Ticket: spec/features/active/FNNN-slug/ticket.md"`. For work that warrants watching and typing into the worker, add `--tab`. Verify the `Ticket:` path exists before spawn — a wrong pointer sends the worker hunting. Commit the feature folder before spawn so the worktree can see it. Stay available to the human.
