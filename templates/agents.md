...
**Second opinion.** When the open question is what to build — interface shape, feature specification, a second read on a decision — rather than whether a branch is correct, spawn an advisor on the Claude engine: `limen spawn --role advisor --engine claude --detached --label "…" '<the question>. Ticket: spec/features/active/FNNN-slug/ticket.md'`. Reach for it when a Pi session would be answering from recall: the Claude job carries whatever tools and skills the coordinator's own machine has, and a question about interface is exactly where that shows. It runs detached in its own worktree, has no interactive tab, and takes no steer. File its final message the way a research report is filed — `advice-<n>.md` in the feature folder, or `spec/research/<slug>/` when the question has no feature yet — strip trailing whitespace, and commit it before acting on it. What it produces is a ticket line, a decision, or a paragraph; the advisor never writes code, commits, merges, or edits the board. Do not reach for it to second-guess an ordinary slice — that is what review is for.
...
After a merge, spawn a picture only when you can name, in one clause in the handoff, a shape that moved — a job kind, a role, a process boundary, or where truth lives. Use the project's chosen model; this example uses the package fallback: `limen spawn --role picture --detached --model gpt-5.6-sol --label "what the diagram now shows" 'Rewrite spec/picture.md. Shape that moved: <one clause>.'`. The job rewrites `spec/picture.md` and nothing else. If that clause will not form, do not spawn. Prompt wording, tests, bugfixes, and ordinary slices are not a picture. Never block the next slice on the picture job.
...
A fan-out is spend the human authorizes. Never start research unprompted. When they ask a question that needs more than one opinion, spawn two researcher jobs, then one judge. Use the board's explicit role-model choices for both researchers and the judge; only where no choice is recorded do these package fallback examples apply:

```bash
limen spawn --role researcher --detached --model gpt-5.6-sol:xhigh --label "…" '…'
limen spawn --role researcher --detached --model grok-4.6:xhigh --label "…" '…'
```
