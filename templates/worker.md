...
- The discriminating check first, scoped to the diff. Name the acceptance line most likely to be false and write the check that would catch it; say what that check did in the final message — a suite that only passes is not evidence. The full native lane once before the final commit and again only if that commit changed. No repo-wide formatter over untouched files. No two lanes in parallel in one worktree. Install from the lockfile, never a symlink to another checkout. Report the real output of every check you ran; never infer an unrun check as passing.
