...
The final live-send receipt and native check results belong in `findings.md` and the external evidence directory. Remaining scope is receiver correlation/authentication repair, deliberate project opt-in, and observed bot-turn proof; none is silently marked fixed by the sender change.
