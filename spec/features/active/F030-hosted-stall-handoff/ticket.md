...
- **Wait means stall.** Herdr `idle`/`done` only starts the stall clock while job `activity` is `wait`. think/tool is the loop still going, including reasoning. Herdr `working` still resets. `blocked` stays immediate. Zero tool calls still do not advisory.
- **Shorter bound.** Default `LIMEN_HOSTED_IDLE_MS` is 60 seconds (F027 shipped 10 minutes). Env still overrides.
