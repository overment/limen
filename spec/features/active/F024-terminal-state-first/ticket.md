...
- **Hosted stop reason.** Hosted jobs have no JSON stream, but `writeHostedResult` already parses the newest session jsonl for the last assistant message — the same pass reads that message's `stopReason` and writes the same `stop-reason` file. A hosted pi that died on a provider error stops looking like a calm `done` with no explanation.
- **Temp-file hygiene.** Finalize sweeps stale `atomicWrite` leftovers (`<file>.<pid>.<hex>.tmp`) from the job directory — a crashed writer's debris does not accumulate in the record.
...
- A hosted session jsonl whose last assistant message carries `stopReason: "error"` yields the same `stop-reason` file at supervisor finalize; a clean hosted session writes none.
- A job directory seeded with a stale `state.12345.abc.tmp` is clean of it after finalize.
